...
AHCC_AUTH_DISABLED=1：存量 API 测试以演示用户身份访问受保护接口（与登录功能
上线前的硬编码行为一致）；真实注册/登录/组隔离流程由 tests/test_auth.py
显式 monkeypatch settings.auth_disabled=False 覆盖。
"""
...
os.environ.setdefault("AHCC_AUTH_DISABLED", "1")
