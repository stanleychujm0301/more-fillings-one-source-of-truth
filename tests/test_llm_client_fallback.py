"""LLM 客户端兜底测试：占位 key 识别 + 主/Ollama 都失败时永不抛异常。"""
...
def test_is_placeholder_key():
    ...
    assert not _is_placeholder_key("ollama")
    assert not _is_placeholder_key(None)  # type: ignore[arg-type]


def test_cached_call_returns_empty_when_all_providers_fail(monkeypatch, tmp_path):
    """主 provider 与 Ollama 兜底都抛异常时，cached_call 返回空值而非抛异常。"""
    monkeypatch.setattr(llm_client.settings, "zhipuai_api_key", "sk-realkey123abc")
    llm_client._CACHE_DIR = tmp_path / "llm_cache"

    fake_primary = MagicMock()
    fake_primary.provider = "zhipu"
    fake_primary.model = "test-model"
    fake_primary.chat_json.side_effect = Exception("primary conn error")
    fake_fallback = MagicMock()
    fake_fallback.provider = "ollama"
    fake_fallback.model = "ollama-model"
    fake_fallback.chat_json.side_effect = Exception("ollama conn error")
    monkeypatch.setattr(llm_client._router, "get", lambda purpose: fake_primary)
    monkeypatch.setattr(llm_client._router, "fallback", lambda: fake_fallback)
...
def test_cached_call_fast_fail_on_placeholder_key(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(llm_client.settings, "zhipuai_api_key", "sk-xxxxxxxxxxxxxxxx")
    llm_client._CACHE_DIR = tmp_path / "llm_cache"
    fake = MagicMock()
    fake.provider = "zhipu"
