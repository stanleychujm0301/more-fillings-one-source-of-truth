...
def test_ui_new_download_links_bust_browser_cache_for_latest_reports():
    ...
    # The ?template=latest query param used to be appended to every report URL but the
    # backend never read it (cache-busting is actually done server-side via no-store
    # response headers), so it was dead weight and has been removed.
